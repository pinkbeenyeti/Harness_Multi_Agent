...
* **1단계**: 코드 수정 전 `structural-sop` 스킬(구조적 사고 7단계 SOP)을 가동해 본질적 병목과 위험을 선제 검증한다. `planner` 역할이 `design_spec.md` 초안을 작성한다(일반=high, 고위험=xhigh). **고위험은 추가로** `critic-standard`(Gemini Flash, high)가 구현 착수 전 계획 자체를 비평(계획비평)한다. 승인 없이는 다음 단계로 넘어가지 않는다.
* **2단계**: `implementer`(Claude Sonnet 5, high)가 구현을 수령한 뒤, `critic-architecture`(Gemini 3.1 Pro)를 연속 가동해 구조 교차비평(Cross-Review)을 강제한다. 이종 모델 교차가 Tier 2의 핵심 안전장치다.
