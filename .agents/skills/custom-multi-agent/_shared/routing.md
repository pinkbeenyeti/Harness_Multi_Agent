...
* **검증**: `validate_result.py`로 `brief.md`(1,800 토큰)와 `result.md`(15,000 bytes / 산문 1,200 토큰)를 린트한다.
...
### 2-A. Tier 2 일반 (파일 수·라인 수·시그니처 변경)

  | 역할 | 모델 | effort |
  |---|---|---|
  | `planner` | GPT-5.6 Sol | high |
  | `implementer` | Claude Sonnet 5 | high |
  | `critic-architecture` | Gemini 3.1 Pro | high |

* **일반 흐름**: `planner` 설계 → `implementer` 구현 → `critic-architecture` 구조 교차비평. 불필요한 중간 비평 단계를 생략하여 기동 속도를 높인다.

### 2-B. Tier 2 고위험 (성능 핫스팟·비가역적 작업)
...
* **1단계 (계획 및 설계)**: 오케스트레이터가 직접 기획하거나 마일스톤을 확정하지 않는다. 브리프를 작성하고 사용자 승인을 받은 뒤 **`planner` 워커**를 기동한다. `planner` 역할은 코드 수정 전 `structural-sop` 관점을 반영하여 본질적 병목과 위험을 선제 검증하고, **요구사항 분석·개선 로드맵·마일스톤 분할·`design_spec.md` 초안**을 작성한다. **고위험 작업에 한해** `critic-standard`(Gemini Flash, high)가 구현 착수 전 계획 자체를 비평(계획비평)한다.
* **2단계 (구현 및 비평)**: `implementer`(Claude Sonnet 5, high)가 구현을 완료하면, `critic-architecture`(Gemini 3.1 Pro, high)를 가동해 구조 교차비평(Cross-Review)을 진행한다. 이종 모델 교차가 Tier 2의 핵심 안전장치다.
  * **3개 파일 이상이면 의존 그룹별로 워커를 병렬 기동한다** (그룹 판정 기준은 SKILL.md §1.3). 그룹 G마다 `result_G.md` → `critic_report_G.md` 쌍을 만들고, G의 result가 나오는 즉시 G의 비평을 시작한다.
...
| 2 일반 | Sol (high) | Sonnet5 (high) | — | Gemini 3.1 Pro (high) |
| 2 고위험 | Sol (high) | Sonnet5 (high) | Gemini Flash (high, 계획비평) | Gemini 3.1 Pro (high) |
