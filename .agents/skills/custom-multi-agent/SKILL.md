...
* **실시간 진행 상태 표기 규칙**: 당신(오케스트레이터)이 새로운 단계(예: 계획 수립, 결과 검증, 코드 병합 등)를 시작할 때에는, 즉시 `task.md`의 `Status` 필드를 `in_progress (Gemini 3.5 Pro Orchestrator is planning...)` 등으로 업데이트하고, `log.md`에 `[DECISION]` 또는 `[VERIFICATION]` 등의 로그를 즉시 덧붙여 어떤 모델이 무엇을 하고 있는지 실시간으로 표시되도록 하십시오. **또한, 파일 변경과 동시에 사용자 채팅 응답(CLI 출력)의 첫 부분에 `⚙️ [Orchestrator Status] 현재 <단계명>을(를) 진행 중입니다...` 형식을 명시적으로 출력하여 사용자가 실시간 진행 상황을 CLI 화면에서도 쉽게 인지할 수 있도록 하십시오.**
