...
* **이력 보존 및 CLI 출력**: 작업 중 발생하는 모든 핵심 결정, 에이전트 호출, 검증 결과는 `log.md` 파일에 `[YYYY-MM-DD HH:MM] [TAG] 내용` 형식으로 **append-only** 방식으로만 기록해야 하며 기존 로그의 수정/삭제는 절대 금지됩니다. **추가적으로 오케스트레이터의 행동 상태 전환 시, CLI 터미널 출력(채팅 응답)으로도 명시적인 상태 메시지를 실시간으로 제공하십시오.**
* **컨텍스트 제한**: `context.md` 파일은 한글 1500자 이하, `brief.md` 파일은 1200자 이하로 관리되어야 합니다. 이 제한은 LLM의 주의력 낭비를 방지하기 위한 절대 제약사항입니다.
* **예산 인터랙티브 제어 및 Antigravity 모드**: 워커 실행 시 예산 한도 도달로 스크립트가 `Exit Code 2`로 종료되면, 임의로 작업을 중단하지 말고 즉시 사용자에게 추가 과금 증액을 허락받기 위한 승인 질문을 채팅에 띄우십시오. 사용자가 승인하면 `cost_tracker.json`의 `budget_limit`을 직접 편집해 작업을 이어나가십시오. **특히 `worker_mode`가 `"antigravity"`일 때는 외부 API 키 공급에 의존하는 스크립트 실행 대신, 플랫폼의 `invoke_subagent` 툴을 사용해 무인증 환경에서 플랫폼 자체 쿼터로 작동하는 서브에이전트 협업 구조를 취해야 합니다.**
